 ...
 *
 * This is the `browser` export condition of `devonian/reflect`: everything
 * in `index.ts` except the Node-only, file-backed `FileIdMap` and
 * `FileKvStore` (see `file.ts`), so it bundles without Node built-ins.
 */
export {
  ReflectionEngine,
  type ReflectionSide,
...
export { InMemoryIdMap, type IdMap, type Link } from './id-map.js';
export { InMemoryKvStore, type KvStore } from './kv-store.js';

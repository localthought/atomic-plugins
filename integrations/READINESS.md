...
| Moneybird (`moneybird`)                        | Drive app `money/moneybird/` over the host relay (contacts only)                                              | Test-side only, like Pets (`New app` + source); catalog install is #94                                                                                                                                                                    | Read-only: contacts of one chosen administration; other collections deferred                                    | Unit and host E2E `money/e2e/moneybird.spec.ts` against a **synthetic** fixture (hand-written, not recorded). Nothing against a real Moneybird account                                                                                                | [#102](https://github.com/ontola/atomic-plugins/issues/102) (recording needs an account), [#94](https://github.com/ontola/atomic-plugins/issues/94)                                             |

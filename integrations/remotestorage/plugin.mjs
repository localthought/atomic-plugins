...
export const manifest = {
  ...
  version: '0.2.0',
  description:
    'Import remoteStorage text exports as Atomic documents and serve explicitly public categories.',
  operations: [],
  secrets: [],
  capabilities: [
    {
      name: 'storage',
      reason:
        'Read scoped Atomic resources; propose reviewed document import intents.',
    },
  ],
  config: {
    key: 'remotestorage',
    properties: {
      table: {
        type: 'string',
        description: 'Parent resource for reviewed text imports',
      },
      publicCategories: {
        type: 'object',
        description:
          'Explicit public category to parent resource mapping; private parents are refused by host read permissions',
      },
    },
    required: ['table'],
  },
  ...
  http: {
    mount: 'drive-prefix',
    reason:
      'Read explicitly exported public text documents. No public write route or private-data authority.',
    routes: [
      {
        id: 'public-storage',
        path: '/storage/public/{*rest}',
        methods: ['GET', 'HEAD'],
        principal: 'anonymous',
        auth: 'none',
...
function validateDocument(doc) {
  ...
  if (/^text\/html(?:;|$)/i.test(doc.contentType))
    fail('HTML cannot be served from the shared drive-prefix origin');
...
export function run(ctx) {
  try {
    ...
    if (!Array.isArray(subjects) || subjects.length > MAX_RECORDS)
      fail('Destination exceeds 128 records');
...
export function readCategory(ctx, category) {
  ...
  for (const subject of subjects) {
    ...
    if (!sameDisplay(resource, doc))
      fail('Local edits require a reviewed text export before serving');
    records.push(doc);
  }

  if (new Set(records.map(d => d.path)).size !== records.length)
    fail('Ambiguous storage path');
  if (
    records.some(a =>
      records.some(b => a !== b && b.path.startsWith(a.path + '/')),
    )
  )
    fail('Document/folder path collision');
...
function folderRepresentation(docs, path) {
  ...
  for (const doc of docs.filter(d => d.path.startsWith(path))) {
    const rest = doc.path.slice(path.length),
      slash = rest.indexOf('/');
    if (slash < 0)
      items[rest] = {
        ETag: etag(doc).slice(1, -1),
        'Content-Type': doc.contentType,
        'Content-Length': utf8(doc.text).length,
      };
  ...
  for (const key of folders)
    items[key] = {
      ETag: folderRepresentation(docs, path + key).tag.slice(1, -1),
    };
...
export function handle(ctx, request) {
  try {
    ...
    if (
      ...
      !request.path.startsWith('/storage/public/')
    )
      return response(
        401,
        'Private storage requires host-verified scoped bearer tokens',
    ...
    const path = request.path
        ...
        .join('/'),
      { pieces, folder } = pathParts(path);
    const category = pieces[1];
    if (!category) return response(404);
    let docs;

    try {
      docs = readCategory(ctx, category);
    } catch {
      return response(503, 'Atomic storage snapshot unavailable');
    }
    ...
    if (folder) {
      ({ body, tag } = folderRepresentation(docs, path));
    ...
    return response(400, 'Invalid remoteStorage request');
  }
}

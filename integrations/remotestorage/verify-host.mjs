...
if (!process.argv[2])
  throw Error('Pass the pinned atomic-server checkout path');
...
).replace('./plugin-manifest-http.js', http);
...
const verdict = run({
  config: { table: 'https://atomic.example/docs' },
  query: () => [],
  read: () => {
...
console.log(
  'Pinned host manifest and verdict parsers accept this bundle contract; no host writes or interoperability performed.',
);

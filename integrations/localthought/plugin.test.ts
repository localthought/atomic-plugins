...
const config = {
  ...
  destinations: {
    issue: {
      ...
      rowClass: 'https://example.com/issue',
    },
  },
  properties: {
    title: 'https://example.com/title',
    number: 'https://example.com/number',
  },
  records: [
    {
      resource: 'issue',
...
it('maps fetched records into typed proposals with scoped identity', () => {
  ...
  expect(result.intents[0]).toMatchObject({
    ...
    set: {
      [config.properties.title]: 'A real issue',
      [config.properties.number]: 42,
    },
  });
});
it('rejects absent fetched records and missing identities', () => {
  expect(() =>
    run({
      config: { ...config, records: undefined },
      query: () => [],
      read: () => ({}),
    }),
  ).toThrow();
...
it('repeated imports preserve local edits and propose no duplicates', () => {
  ...
  const saved: Record<string, unknown> = {
    ...
    'https://atomicdata.dev/properties/isA': [
      config.destinations.issue.rowClass,
    ],
  };
  const host = {
    config,
    query: (p: string, v: string) =>
      saved[p] === v ? ['https://example.com/row'] : [],
    read: () => saved,
  };

...
const share = { name: 'Design.md', providerId: 'share-123', owner: 'alice@cloud.example', sender: 'alice@cloud.example', shareWith: 'bob@atomic.example', shareType: 'user', resourceType: 'file', protocol: { name: 'multi', webdav: { uri: 'remote.php/dav/share-123', permissions: ['read'], sharedSecret: 'DO-NOT-PERSIST' } } };
function host(config = {}, rows = {}) {
  return {
    config: { publicOrigin: 'https://atomic.example', mode: 'import-reviewed-share', peerOrigin: peer, allowedPeers: [peer], recipient: share.shareWith, document: doc, shareJson: JSON.stringify(share), ...config },
    read(s) { return rows[s] ?? (s === doc ? { [P.isA]: ['https://atomicdata.dev/classes/DocumentV2'] } : undefined); },
    query(p, value) { return Object.keys(rows).filter(s => rows[s][p] === value); },
  };
}
...
test('disabled discovery is explicit and never advertises unsupported file exchange', () => {
  ...
  assert.deepEqual(JSON.parse(result.body), { enabled: false, apiVersion: '1.3.0', endPoint: 'https://atomic.example/ocm', resourceTypes: [] });
  assert.equal(handle(host(), { method: 'HEAD', path: '/ocm-provider' }).body, '');
  assert.equal(handle(host({ publicOrigin: '' }), { method: 'GET', path: '/ocm-provider' }).status, 503);
});
test('receive fails closed even with invented caller and malicious payload', () => {
  const result = handle(host(), { method: 'POST', path: '/ocm/shares', caller: 'admin', body: JSON.stringify(share) });
  assert.equal(result.status, 501);
  assert.equal(result.intents, undefined);
  assert.equal(handle(host(), { method: 'GET', path: '/ocm/shares' }).status, 405);
...
test('protocol parser drops WebDAV credentials and locations, preserves exact expiration', () => {
  const parsed = parseShare(JSON.stringify({ ...share, expiration: 1800000000 }));
...
test('durable duplicate is a no-op across fresh runs; modifications conflict', () => {
  ...
  assert.deepEqual(run(host({}, { receipt: row })), { intents: [], problems: [] });
  fail({ shareJson: JSON.stringify({ ...share, name: 'Changed' }) }, /conflicts/, { receipt: row });
  fail({}, /conflicts/, { receipt: { ...row, [P.about]: 'https://atomic.example/other' } });
...
test('operator policy, scope and document type are required before any intent', () => {
  ...
  fail({}, /accessible Atomic/, { [doc]: { [P.isA]: ['https://atomicdata.dev/classes/File'] } });
  const c = host(); c.read = () => { throw new Error('host denied read'); };
  assert.deepEqual(run(c).intents, []);
});
test('bounded parsing refuses malformed, oversized, unsupported and ambiguous wire data', () => {
  for (const body of ['{', 'null', '[]', ' '.repeat(16385)]) assert.throws(() => parseShare(body));
  for (const patch of [{ name: '\n' }, { shareType: 'group' }, { resourceType: 'folder' }, { expiration: 1.2 }, { protocol: null }, { providerId: 7 }])
    assert.throws(() => parseShare(JSON.stringify({ ...share, ...patch })));
  assert.throws(() => parseShare(JSON.stringify({ ...share, protocol: { name: 'multi', webdav: { uri: 'x', permissions: ['read'], requirements: ['must-use-mfa'] } } })), /requirements/);
});
test('origin parsing works without URL and refuses credentials, query and path', () => {
  assert.equal(origin(peer), peer);
  for (const bad of ['http://cloud.example', 'https://user:pw@cloud.example', peer + '/path', peer + '?token=secret', 'https://cloud.example\n']) assert.throws(() => origin(bad));
});
test('self-contained release rebuilds reproducibly with executable manifest routes', async () => {
  ...
  assert.equal(await readFile(new URL('./plugin.js', import.meta.url), 'utf8'), before);
  const built = await import('data:text/javascript;base64,' + Buffer.from(before).toString('base64'));
  ...
  for (const route of manifest.http.routes) {
    ...
    assert.notEqual(handle(host(), { path: route.path, method: route.methods[0] }).status, 404);
  }
});

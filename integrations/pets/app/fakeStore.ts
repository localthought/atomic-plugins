 ...
 * app -> table. `proxy` answers from the same Pets fixture the mock
 * integration proxy serves (`../fixtures/pets/scenario.mjs`), with its
 * Link-header pagination. Test-only; not bundled.
 */
import { petsFixture } from '../fixtures/pets/scenario.mjs';
import type {
  HostProxy,
  HostProxyRequest,
  JSONValue,
...
export function fakeStore({
  ...
  const proxy: HostProxy = {
    async request(request) {
      calls.push(request);
      const url = new URL(request.path, 'https://pets.example');
      const result = pets.request(request.method ?? 'GET', url);

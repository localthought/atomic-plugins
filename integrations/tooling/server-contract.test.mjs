...
import {
  copyFileSync, mkdtempSync, mkdirSync, writeFileSync, rmSync,
} from 'node:fs';
...
function fixture(t) {
  ...
  writeFileSync(resolve(base, 'integrations/lanes.json'), JSON.stringify(config));
...
test('contracts reject identity drift, invalid capabilities and implementation claims', () => {
  ...
  for (const change of [
    { id: 'other' }, { schemaVersion: 2 }, { runtime: 'native-rust' },
    { status: 'implemented' }, { name: '' }, { scope: '  ' },
    { firstInteropMilestone: '' }, { proposedHostCapabilities: [] },
  ...
  ]) assert.throws(() => validateContract({ ...contract, ...change }, contract.id, readme));
});
...
test('headings may add context and prose may wrap across lines', () => {
  const wrapped = readme
    ...
    .replace(contract.firstInteropMilestone, contract.firstInteropMilestone.replaceAll(' ', '\n'));
...
test('documentation must disclose status, runtime, scope, milestone and reproduction', () => {
  for (const required of [
    ...
    '## Implementation checklist', '## CI', '**scaffold**', 'QuickJS',
    contract.scope, contract.firstInteropMilestone,
    `node integrations/tooling/run-lane.mjs ${contract.id}`,
  ]) assert.throws(() => validateContract(contract, contract.id, readme.replace(required, '')));
});
...
test('real runner executes the contract and propagates failures without browser dependencies', t => {
  ...
  for (const file of [
    'run-lane.mjs', 'lanes.mjs', 'server-contract.mjs', 'serve.mjs',
    'link-atomic-server.mjs', 'deps.mjs',
  ]) copyFileSync(resolve(root, 'integrations/tooling', file), resolve(tooling, file));
  ...
  writeFileSync(resolve(folder, 'pnpm-lock.yaml'), 'intentionally invalid fixture');
  ...
  for (const args of [[], ['--tier', 'contract']]) {
    ...
    assert.match(result.stdout, /scaffold contract valid; protocol implementation and interoperability not tested/);
    assert.match(result.stdout, /contract passed/);
    assert.equal(result.stderr, '');
  }
  writeFileSync(resolve(folder, 'plugin.json'), JSON.stringify({ ...contract, runtime: 'native-rust' }));
  const rejected = run();

...
3. **Fix an event** — retitle, move, add a location or notes, and be sure the
   ...
   for event detail. Our addition is an explicit *review before send* step,
...
- **Tokens.** Components use only `--pl-*`, each mapped to the host variables
  ...
  | `--pl-pos` | *(none in host)* | `#1f7a45` | `#56c98a` |
...
- **Header row** `[icon + "Calendar" | source chips | status pill | primary action]`.
  For Calendar the source chips are the *visible calendars*, each a colour
...
- *Both changed* — field rows with two options, "Keep mine" / "Use Google's",
  none preselected; "Resolve" is enabled when every field has a choice.
- *Missing in Google* (cancelled, now recurring, or inaccessible) — "This
  ...
  and it is local).
- *Missing or rebound local row* — shown with "Open row in table"; no
  automatic fix.
...
| 403 | "Google refused access to *Work*. You may have lost access to this calendar." | Choose calendars |
| 404 on a calendar | "Calendar *Team offsite* no longer exists or isn't shared with you." | Remove from list |
| 429 | "Google is limiting requests. Retrying in 40 s." (from `retry-after`) | Retry now |
| 5xx / network | "Couldn't reach Google." | Retry |
| > 25,000 events | "*Work* has more than 25,000 events, so the import stopped." | — |

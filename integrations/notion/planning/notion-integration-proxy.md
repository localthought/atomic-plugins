...
> **Historical record, moved from atomic-server's `planning/`** (removed there
> by `claude/remove-notion-code`), with `notion-proxy/` next to it. Paths below
> are atomic-server paths as of 2026-09-06. The current plan is `../README.md`
> and issue #8.
...
`planning/notion-proxy/` (now `notion-proxy/` next to this file). They need publication with immutable OAD/overlay pins,

...
const STATUS = [o('s1', 'Not started', 'default'), o('s2', 'In progress', 'blue'), o('s3', 'Done', 'green')];
const READ = [o('r1', 'To read', 'gray'), o('r2', 'Finished', 'green')];
const P = (id: string, name: string, type: string, options?: typeof STATUS): SchemaProperty => ({
  id,
  name,
  type,
...
const roadmap = report('d1', 'Roadmap', [
  P('title', 'Name', 'title'),
...
const row = (
  ...
  lastEdited: number,
  values: Row['values'],
): Row => ({ subject, name, pageId: subject.replace('row', 'p'), dataSource, url: `https://www.notion.so/${subject}`, lastEdited, values });

const rows: Row[] = [
  row('row1', 'Launch plan', 'Roadmap', NOW - 1000, { 'notion-st': 's2', 'notion-pt': 3, 'notion-dn': false }),
  row('row2', 'Write changelog', 'Roadmap', NOW - 5000, { 'notion-st': 's3', 'notion-pt': 0, 'notion-dn': true }),
  row('row3', 'Retrospective', 'Roadmap', NOW - 9000, { 'notion-st': 'gone-option' }),
  row('row4', 'Thinking in Systems', 'Reading list', NOW - 3000, { 'notion-st2': 'r1', 'notion-ln': 'https://example.org/book' }),
...
describe('view model', () => {
  it('shows a database’s own columns in Notion order, without skipped types', () => {
    ...
    expect(cols.map(c => c.name)).toEqual(['Name', 'Status', 'Points', 'Done', 'Last edited in Notion']);
  });

  it('shows in "All" only the columns every database has by name and type', () => {
    const cols = columnsFor(ALL, sources(rows, last));
    expect(cols.map(c => c.name)).toEqual(['Name', 'Database', 'Status', 'Last edited in Notion']);
  ...
  it('searches titles and option names, not option ids', () => {
    const cols = columnsFor(ALL, sources(rows, last));
    expect(searchRows(rows, cols, 'progress').map(r => r.name)).toEqual(['Launch plan']);
    expect(searchRows(rows, cols, 's2')).toEqual([]);
    expect(searchRows(rows, cols, 'THINKING').map(r => r.name)).toEqual(['Thinking in Systems']);
  });

  it('sorts ascending, descending, then off; options in Notion order; empties last', () => {
    const cols = columnsFor('Roadmap', sources(rows, last));
    expect(nextSort(null, 'notion-pt')).toEqual({ key: 'notion-pt', dir: 'asc' });
    expect(nextSort({ key: 'notion-pt', dir: 'asc' }, 'notion-pt')).toEqual({ key: 'notion-pt', dir: 'desc' });
    expect(nextSort({ key: 'notion-pt', dir: 'desc' }, 'notion-pt')).toBeNull();
    const roadmapRows = rows.filter(r => r.dataSource === 'Roadmap');
    expect(sortRows(roadmapRows, cols, { key: 'notion-pt', dir: 'desc' }).map(r => r.name)).toEqual([
      'Launch plan', 'Write changelog', 'Retrospective', 'Untitled draft',
    ]);
    expect(sortRows(roadmapRows, cols, { key: 'notion-st', dir: 'asc' }).map(r => r.name)).toEqual([
      'Launch plan', 'Write changelog', 'Retrospective', 'Untitled draft',
    ]);
    expect(sortRows(roadmapRows, cols, { key: 'edited', dir: 'desc' })[0]!.name).toBe('Launch plan');
  });

  it('groups a board in Notion’s option order, unknown options after, "No value" last', () => {
    const cols = columnsFor('Roadmap', sources(rows, last));
    const status = cols.find(c => c.name === 'Status')!;
    const groups = groupRows(rows.filter(r => r.dataSource === 'Roadmap'), status);
  ...
  it('says what the pill says per state', () => {
    ...
    expect(pill({ ...ready, last: { ...last, dataSources: [reading] } }, NOW)).toEqual({ tone: 'ok', text: 'Synced 4 min ago' });
    expect(pill({ kind: 'syncing', connectionId: 'c', rows, last, progress: [
      { dataSource: 'd1', title: 'Roadmap', phase: 'done', pages: 4 },
      { dataSource: 'd2', title: 'Reading list', phase: 'reading', pages: 1 },
    ] }, NOW)).toEqual({ tone: 'sync', text: 'Syncing… Reading list' });
    expect(pill({ kind: 'importing', connectionId: 'c', rows: [], progress: [
      { dataSource: 'd1', title: 'Roadmap', phase: 'done', pages: 4 },
      { dataSource: 'd2', title: 'Reading list', phase: 'reading', pages: 1 },
      { dataSource: 'd3', title: 'Hiring', phase: 'listing', pages: 0 },
    ] }, NOW)?.text).toBe('Importing 2 of 3 databases');
    expect(pill({ kind: 'reauth', rows }, NOW)).toEqual({ tone: 'neg', text: 'Reconnect needed' });
    expect(pill({ kind: 'no-databases', connectionId: 'c', rows: [] }, NOW)?.text).toBe('No databases shared');
    expect(pill({ kind: 'failed', connectionId: 'c', rows, title: '', message: '', technical: '' }, NOW)?.text).toBe('Sync failed');
...
describe('view (DOM)', () => {
  ...
  beforeEach(() => {
    ...
    app = createApp(root, actions, { now: () => NOW, locale: 'en-GB', width: () => width });
  });
  ...
  it('S1: a host without the relay explains and offers no action', () => {
    ...
    expect(q('h2')?.textContent).toMatch(/can’t connect apps to other services/);
  ...
  it('S2: first run offers exactly one action, Connect Notion', () => {
    ...
    expect(q('h2')?.textContent).toBe('Bring your Notion databases into Atomic');
  ...
  it('S4: first import lists each database’s progress', () => {
    app.render({ kind: 'importing', connectionId: 'c', rows: [], progress: [
  ...
  it('S5: nothing shared offers "Choose pages in Notion"', () => {
    app.render({ kind: 'no-databases', connectionId: 'c', rows: [], last: { ...last, dataSources: [] } });
  ...
  it('S6: a real table with scoped headers, sort state and typed cells', () => {
    ...
    expect(all('th').map(th => th.getAttribute('scope'))).toEqual(Array(5).fill('col'));
    expect(all('th').map(th => th.textContent)).toEqual(['Name', 'Status', 'Points', 'Done', 'Last edited in Notion']);
    ...
    expect(cells[3]!.querySelector('[role=img]')?.getAttribute('aria-label')).toBe('No');
    expect(cells[4]!.textContent).toMatch(/^Today, /);
    // An option id the schema does not know is not shown as an id.
    const retro = all('tbody tr').find(tr => tr.textContent?.startsWith('Retrospective'))!;
  ...
  it('filters with search after a 150 ms pause', () => {
    ...
    expect(all('tbody tr').map(tr => tr.querySelector('td')!.textContent)).toEqual(['Write changelog']);
  ...
  it('S8: board is off on "All" and groups one database by status', () => {
    app.render(ready);
    expect(q<HTMLButtonElement>('[data-key="view:board"]')!.disabled).toBe(true);
    q<HTMLButtonElement>('[data-key="chip:Roadmap"]')!.click();
  ...
  it('S10: sync details group warnings per database', () => {
    ...
    expect(details.textContent).toContain('1 page has formatting in Notes, so Notes was not copied for it.');
    expect(details.querySelector('details summary')?.textContent).toBe('Technical details');
  });
  ...
  it('S11–S13: one banner with one recovery action; role=alert only after a sync', () => {
    ...
    expect(all('.pl-banner button').map(b => b.textContent)).toEqual(['Reconnect Notion']);
    expect(q('[data-key=sync-now]')).toBeNull();

    app.render({ ...ready, kind: 'syncing', progress: [] });
    app.render({ kind: 'failed', connectionId: 'c', rows, last, title: 'Notion didn’t answer properly', message: 'Error (502).', technical: 'POST /v1/search → 502' });
  ...
  it('S12: rate-limited retries by itself at retry-after', () => {
    vi.useFakeTimers();
    app.render({ kind: 'rate-limited', connectionId: 'c', rows, last, retryAt: NOW + 30_000, pagesRead: 31, technical: '' });

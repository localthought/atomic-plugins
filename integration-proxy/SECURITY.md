...
Issue #40; the wire format is in the README. What the proxy relies on, and what it does not yet provide:

- **Authority.** A connection bound to `did:ad:agent:{pubkey}` accepts an Ed25519 signature by that key. The key is taken from the DID itself; nothing is fetched. The proxy never sees the key's secret half. Per the #40 discussion, the intended DID is the plugin's own app agent, not the person's, so revocation and attribution are per plugin. The proxy cannot tell the two apart; that is the connecting hub's choice.
- **Signed messages are domain-separated.** Both formats start with a versioned `integration-proxy-…-v1` line and contain newlines. A request message ends in a hex digest, not a number, so none can be read as Atomic's own `"{subject} {timestamp}"` authentication message, and a capability signature does not verify as a request signature.
- **Replay.** A signed request is accepted once within its ±5-minute window. A digest of the message, not of the signature, goes into `used_challenges`, which is kept for 10 minutes. A capability is a bearer token until `exp`, which is at most 15 minutes away. A leaked capability is usable until then, for one connection and one platform. Losing one costs a new signature, not an OAuth flow.
- **Row integrity.** Each row's envelope is sealed with associated data naming its `connection_id`, so envelopes cannot be swapped between rows. Refreshes take a 30-second lease column on the row, so concurrent callers cannot spend one refresh token twice.
- **Lifetime.** A row is removed after 90 days (`CONNECTION_IDLE_DAYS`) without an authenticated request. Only authenticated use refreshes that clock; knowing a `connection_id` does not.
- **Not yet provided.** (1) Revoking one connection. `REVOKED_SUBJECTS` revokes a tenant or a DID, which covers all of its connections. (2) Proof that the connecting hub holds the DID's key. `user_id` is still the hub's assertion. Binding a grant to a DID someone else holds gives the binder nothing, because they cannot sign for it. It does let a hub attach its grant to another agent. (3) A second node for the same app agent. If that node lacks the key, see ontola/atomic-plugins#41; its requests fail closed with `401`.
